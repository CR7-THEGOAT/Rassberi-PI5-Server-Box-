...
// REVISION 22: REV21 pull handles plus VCOM USB-to-SATA cable clearance.
...
// REV21: Each tray gets a flush, open U-frame pull handle.  It is a single
// printed part of the tray, so there are no separate hinges, screws, labels,
// or protrusions beyond the established front face of the enclosure.
tray_handle_face_depth = 4.0;
tray_handle_outer_w = 70.0;
tray_handle_outer_h = 15.4;
tray_handle_outer_corner_r = 2.2;
tray_handle_open_w = 46.0;
tray_handle_open_h = 7.6;
tray_handle_open_corner_r = 1.4;
...
// A 2D rounded rectangle used in the tray's X/Z front plane.  Keeping the
// outer face and the grip opening separately dimensioned makes the handle
// strong enough for a loaded 2.5-inch drive without changing the slide fit.
...
module tray_pull_handle_face(tw, front_h) {
    assert(tray_handle_outer_w < tw && tray_handle_outer_h < front_h,
           "Tray handle must stay inside the front opening");
    // The face reaches exactly to the established outer face of the box when
    // seated.  A 0.1 mm overlap with the original tray front gives one solid
    // printable body instead of a touching-only surface.
    translate([tw/2, 0.1, front_h/2]) rotate([90,0,0])
        linear_extrude(height=tray_handle_face_depth+0.1)
            tray_handle_rounded_rect_2d(tray_handle_outer_w,
                                        tray_handle_outer_h,
                                        tray_handle_outer_corner_r);
}
...
module hdd_tray() {
    ...
    difference() {
        union() {
            ...
            cube([tw, 3, front_h]);
            // Rounded, flush front pull handle inspired by the supplied
            // drive-caddy reference.  It stays within the existing tray bay.
            tray_pull_handle_face(tw, front_h);
        ...
        // Full-depth rounded pull opening replaces the old shallow finger
        // pocket.  The opening is 46.0 x 7.6 mm; its 12 mm side rails and
        // 3.9 mm top/bottom rails retain a stiff U-frame grip.
        translate([tw/2,3.2,front_h/2]) rotate([90,0,0])
            linear_extrude(height=tray_handle_face_depth+3.4)
                tray_handle_rounded_rect_2d(tray_handle_open_w,
                                            tray_handle_open_h,
                                            tray_handle_open_corner_r);
...
module base() {
    difference() {
        ...
        // The board envelope remains 103 x 73 mm with 5 mm PCB/metal base.
        ...
        // lower Ethernet row.  The 17 mm value is the front-to-back
        // clearance envelope; the visible opening remains circular.
        translate([dc_jack_x, body_y-port_depth, dc_jack_z])
            rotate([-90,0,0]) cylinder(h=port_depth+2, r=dc_jack_r);

        // Five lower RJ45 openings aligned with the five-port Ethernet board.
        // The former upper rectangular row is intentionally gone; the round
        // spare hole below remains unchanged and is not an Ethernet port.
        ...
        // Two-inch air-conditioning/capacitor opening next to the lower
        // Ethernet row.  Its lower edge is 3 mm above the Ethernet openings;
        // the circle remains separate and is not converted to RJ45.
...
module top() {
    difference() {
        ...
        // Centered 1-inch screen wire opening, preserved separately.
